...
FILTER_TYPE_OPTIONS = (("tags", "Tags"), ("search", "Search"))
...
class Tag(models.Model):
    ...
    def get_top_10_recent_tags():
        ...
        """
        recent_tags = (
            Tag.objects.filter(notetag__note__isnull=False)
            .annotate(
                last_used=Note.objects.filter(notetag__tag=models.OuterRef("pk"))
                .order_by("-update_date")
                .values("update_date")[:1]
            )
            .order_by("-last_used")
            .distinct()[:10]
        )
...
class Note(models.Model):
    ...
    link_check_fail_count = models.PositiveIntegerField(default=0)
    favourite = models.BooleanField(default=False)
