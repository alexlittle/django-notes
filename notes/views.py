...
class HomeView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["overdue"] = (
            base_query_dated.exclude(status="completed")
            .filter(due_date__lt=user_aware_now)
            .order_by("priority_order", "due_date")
        )
        context["reminder"] = self._filter_for_reminders(
            base_query_dated.exclude(status="completed").order_by("due_date")
        )
        queryset = base_query_dated.filter(due_date=user_aware_now).order_by(
            "-status_order", "priority_order", "title"
        )
        context["today"] = queryset
        context["tomorrow"] = base_query_dated.filter(
            due_date=user_aware_now + timedelta(days=1)
        ).order_by("-status_order", "priority_order", "title")
        context["next_week"] = base_query_dated.filter(
            due_date__gt=user_aware_now + timedelta(days=1),
            due_date__lte=user_aware_now + timedelta(days=7),
        ).order_by("due_date", "priority_order", "title")
        context["next_month"] = base_query_dated.filter(
            due_date__gt=user_aware_now + timedelta(days=7),
            due_date__lte=user_aware_now + timedelta(days=31),
        ).order_by("due_date", "priority_order", "title")
...
class FutureTasksView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        user_aware_now = get_user_aware_date(self.request.user)
        base_query_dated = Note.objects.filter(
            user=self.request.user,
            due_date__isnull=False,
            type="task",
            status__in=["open", "inprogress", "completed"],
        ...
        if not showall:
            base_query_dated = base_query_dated.exclude(status="completed")
        context["future"] = base_query_dated.filter(
            due_date__gt=user_aware_now + timedelta(days=31)
        ).order_by("due_date")
...
class TasksTagsView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        base_query_dated = Note.objects.filter(
            user=self.request.user, due_date__isnull=False, type="task"
        )
        base_query_undated = Note.objects.filter(
            user=self.request.user, due_date__isnull=True, type="task"
        )
        if not showall:
        ...
        context["tags_dated"] = (
            Tag.objects.filter(user=self.request.user, notetag__note__in=base_query_dated)
            .distinct()
            .annotate(note_count=Count("notetag__note"))
            .order_by("name")
        )
        context["tags_undated"] = (
            Tag.objects.filter(user=self.request.user, notetag__note__in=base_query_undated)
            .distinct()
            .annotate(note_count=Count("notetag__note"))
            .order_by("name")
        )
...
class EditView(TemplateView):
    def get(self, request, note_id):
        note = Note.objects.get(user=self.request.user, pk=note_id)
        tags = Tag.objects.filter(user=self.request.user, notetag__note=note).values_list(
            "name", flat=True
        )
        data = {}
        data["tags"] = ", ".join(tags)
...
class TagView(ListView):
    ...
    def get_queryset(self):
        ...
        return get_filtered_notes(self.request.user, self.kwargs["tag_slug"]).order_by(
...
class SearchView(ListView):
    ...
    def get_queryset(self):
        ...
        if search_query:
            search_id_results = CombinedSearch.objects.combined_search(
                self.request.user.id, search_query
            )
...
class FavouritesView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["filters"] = [
            (fn, fn.get_count(self.request.user)) for fn in SavedFilter.objects.all()
        ]
        return context
...
class TagsView(ListView):
    ...
    def get_queryset(self):
        ...
        return (
            ...
            .annotate(count=Count("notetag__note__id"))
...
class ScheduleView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for tag_slugs in row_defs:
            ...
            tags_qs = {t.slug: t for t in Tag.objects.filter(slug__in=tag_slugs)}
            row_tag_objs = [tags_qs[slug] for slug in tag_slugs if slug in tags_qs]

            row_tags.append(
                {
                    "key": row_key,
                    "label": " / ".join((t.label or t.slug) for t in row_tag_objs),
                    "first_label": (row_tag_objs[0].label or row_tag_objs[0].slug)
                    if row_tag_objs
                    else tag_slugs[0],
                    "slugs": tag_slugs,
                }
        ...
        context["grid"] = grid
        context["row_tags"] = sorted(row_tags, key=lambda r: r["first_label"].lower())
        return context
