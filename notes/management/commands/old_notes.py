...
class Command(BaseCommand):
    help = _("Looks for very old notes")
    ...
    def handle(self, *args, **options):
        ...
        start_cut_off_date = timezone.now() - dateutil.relativedelta.relativedelta(
            years=start_years
        )
        end_cut_off_date = start_cut_off_date - dateutil.relativedelta.relativedelta(years=no_years)
        old_notes = Note.objects.filter(
            create_date__lte=start_cut_off_date, create_date__gte=end_cut_off_date
        )
